...
export const getAllBlogs = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (status) {
    ...
    res
      .status(200)
      .json({ message: "All blogs available by this selected status", status: true, data: { blogs: blogsByStatus } });
    return;
  }
...
export const updateBlogStatus = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const author = await getUserById(updatedBlog.authorId);
  const subject = `Blog status updated to ${updatedBlog.status}`;
  const message = `Hi ${author.username}, your blog titled "${updatedBlog.title}" has been ${status}.`;

  await sendEmail(author.email, subject, message);
